...
var filter_results = 0; // Результат - индекс наиболее кликабельного checkbox-а.      windows.filter_results
...
(function() {
    ...
    function handler(event) {
        ...
        if (pressedCheckBox.checked) { // смотрим только включение
            var lastIndex;
            ...
            var tmpObj;
            tmpObj = JSON.stringify(counterOfCheked); // Подготовка для записи в кукис
    ...
    function init() { // инициализация, считывание с кукис предидущих значений и навешивание обработчика событий
        var str;
        str = getCookie('chbox');
    ...
    onReady(init);
    bindEvent(document, 'change', handler);
})(filter_results);